...
-- 🔥 ĐẶT HÀM Ở ĐÂY (NGAY SAU LIBRARY)
local HttpService = game:GetService("HttpService")
local TeleportService = game:GetService("TeleportService")

local function HopServerByAPI(url, condition)
    local success, data = pcall(function()
        return game:HttpGet(url)
    end)

    if not success then
        warn("Lỗi load API:", url)
        return
    end
...
Status:AddButton({
    "Hop Full Moon",
    function()
        local url = "https://raw.githubusercontent.com/Naknohack/Hop/refs/heads/main/Fullmoon.json"

        HopServerByAPI(url, function(v)
            -- bạn có thể chỉnh điều kiện ở đây
            return v.Players <= 6 -- ưu tiên server ít người
        end)
    end
})
...
Status:AddButton({
    "Hop Darkbeard",
    function()
        local url = "https://raw.githubusercontent.com/Naknohack/Hop/refs/heads/main/Darkbeard.json"
...
Status:AddButton({
    "Hop Legendary Sword",
    function()
        local url = "https://raw.githubusercontent.com/Naknohack/Hop/refs/heads/main/SwordLegendary.json"

        HopServerByAPI(url, function(v)
            -- điều kiện lọc server
            return v.Players <= 5 -- ít người (dễ kiếm sword)
        end)
    end
})
Status:AddTextBox({
...
Event:AddToggle({
    ...
    Description = "Ngồi lên thuyền sẽ tự chạy",
...
vu497.RenderStepped:Connect(function()
    ...
    local char = vu496.LocalPlayer.Character
    ...
    -- tìm thuyền mình đang ngồi
    local mySeat = nil
    for _, boat in pairs(vu499.Boats:GetChildren()) do
    ...
    if mySeat then
        mySeat.MaxSpeed = vu502

        local target = CFrame.new(-16217.7568, 9.1267, 446.0653)
        local pos = mySeat.Position
        local targetPos = target.Position

        -- di chuyển
        local move = (targetPos - pos).Unit * mySeat.MaxSpeed * vu497.RenderStepped:Wait()
        mySeat.CFrame = mySeat.CFrame + move

        -- quay đầu thuyền
        mySeat.CFrame = CFrame.new(mySeat.Position, targetPos)

        -- tới nơi thì dừng
        if (mySeat.Position - targetPos).Magnitude < 120 then
