...
v485:AddDropdown({
    ...
    Callback = function(p36)
        if p36 == "Small" then
            vu32:SetScale(900)
        elseif p36 == "Normal" then
            vu32:SetScale(450)
        elseif p36 == "Bigger" then
            vu32:SetScale(360)
        elseif p36 == "Extremely large" then
            vu32:SetScale(300)
        else
            vu32:SetScale(450)
        end
...
task.spawn(function()
    while task.wait(1) do
        if not _G.IslandESP then continue end
        ...
        for _,v in pairs(workspace:GetDescendants()) do
            ...
            and not game.Players:FindFirstChild(v.Name)
            and v.Name ~= "Camera"
            and v.Name ~= "Terrain" then

                local part = v:FindFirstChildWhichIsA("BasePart")
                if not part then continue end

                if not part:FindFirstChild("IslandESP") then
                    local gui = Instance.new("BillboardGui")
                    gui.Name = "IslandESP"
                    gui.Size = UDim2.new(0,200,0,50)
                ...
                local dist = math.floor((rootPos - part.Position).Magnitude)
                part.IslandESP.TextLabel.Text = v.Name .. " [ "..dist.."m ]"
            end
        end
    end
end)
...
-- Biến global
getgenv().EspIslands = false

-- ESP tạo
local function AddESP(Part, ESPColor)
    if Part:FindFirstChild("ESP_REDzHUB") then return end

    local BHA = Instance.new("BoxHandleAdornment")
    BHA.Name = "ESP_REDzHUB"
    BHA.Adornee = Part
    BHA.Size = Part.Size
    BHA.Color3 = ESPColor or Color3.fromRGB(0,255,255)
    ...
    TL.TextColor3 = ESPColor or Color3.fromRGB(0,255,255)
...
function EspIslands()
    while getgenv().EspIslands do
        ...
        if Locations and Locations:FindFirstChild("Locations") then
            for _,Island in pairs(Locations.Locations:GetChildren()) do
                if Island:IsA("Model") or Island:IsA("Part") then
                    AddESP(Island)
                end
            end
        end
