...
local SmoothX, SmoothY, SmoothSide = nil, nil, nil
...
local function ControlSize()
	...
	local TargetX = math.clamp(Pos1.X.Offset, 430, 1000)
	local TargetY = math.clamp(Pos1.Y.Offset, 200, 500)
	local TargetSide = math.clamp(Pos2.X.Offset, 135, 250)

	SmoothX = SmoothX or TargetX
	SmoothY = SmoothY or TargetY
	SmoothSide = SmoothSide or TargetSide

	local smoothSpeed = Dragging and 0.15 or 0.08

	SmoothX = Lerp(SmoothX, TargetX, smoothSpeed)
	SmoothY = Lerp(SmoothY, TargetY, smoothSpeed)
	SmoothSide = Lerp(SmoothSide, TargetSide, smoothSpeed)

	MainFrame.Size = UDim2.fromOffset(SmoothX, SmoothY)
	MainScroll.Size = UDim2.new(0, SmoothSide, 1, -TopBar.Size.Y.Offset)
	Containers.Size = UDim2.new(1, -SmoothSide, 1, -TopBar.Size.Y.Offset)

end

RunService.RenderStepped:Connect(ControlSize)
