...
const dictionaryFile = 'Tudien.txt';
...
async function startNight(guildId, game) {
    ...
    setTimeout(() => processNight(guildId, game), 45000);
function processNight(guildId, game) {
    ...
    let thosanArray = Array.from(deadThisNight);
...
async function startMorning(guildId, game, deadPlayers) {
    ...
    const morningEmbed = new EmbedBuilder()
        .setTitle('🌅 TRỜI SÁNG 🌅')
        .setDescription(`${desc}\n\nCác bạn có **1 phút** để thảo luận tìm ra Ma Sói trước khi bỏ phiếu treo cổ.`)
        .setColor('#f1c40f');
    await game.channel.send({ embeds: [morningEmbed] });
    
    if (!checkWinCondition(guildId, game)) {
        setTimeout(() => startVote(guildId, game), 60000);
    }
}
...
async function startVote(guildId, game) {
    ...
    const voteMsg = await game.channel.send({ 
        content: '⚖️ **THỜI GIAN BỎ PHIẾU (60 giây):** Hãy chọn người bạn nghi ngờ nhất!', 
        components: [row] 
    });
    ...
    collector.on('end', () => processVoteResult(guildId, game, votes));
}

function processVoteResult(guildId, game, votes) {
    ...
    for (const [target, count] of Object.entries(votes)) {
        if (count > highestVoteCount) {
            ...
            hangedUser = target;
            tie = false;
        } else if (count === highestVoteCount) tie = true;
    }

    if (tie || hangedUser === 'skip' || hangedUser === null) {
        game.channel.send('⚖️ Không ai bị treo cổ hôm nay do hòa phiếu hoặc đa số chọn Skip!');
    } else {
        game.playerRoles[hangedUser].alive = false;
        game.channel.send(`⚖️ Toàn dân đã thống nhất đưa <@${hangedUser}> lên giá treo cổ!`);
        ...
        if (game.playerRoles[hangedUser].role === 'Thợ săn') {
            ...
            if (aliveOthers.length > 0) {
                const randomTarget = aliveOthers[Math.floor(Math.random() * aliveOthers.length)];
                game.playerRoles[randomTarget].alive = false;
                game.channel.send(`🔫 **Thợ săn** <@${hangedUser}> phẫn nộ nổ súng kéo theo <@${randomTarget}> chết cùng trên giá treo cổ!`);
            }
        }
    }

    if (!checkWinCondition(guildId, game)) {
        setTimeout(() => startNight(guildId, game), 5000);
    }
}
