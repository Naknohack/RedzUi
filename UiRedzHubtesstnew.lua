...
WindowScale.Scale = redzlib.Save.UIScale or 1

-- SAVE INSTANCE
redzlib.Instances = redzlib.Instances or {}
redzlib.Instances.MainFrame = MainFrame
Library = Library or {}

function Library:MakeTab(config)
	return redzlib:MakeTab(config)
end
...
WindowScale.Parent = MainFrame
    ...
    Size = UDim2.new(1, -(redzlib.Save and redzlib.Save.TabSize or 300), 1, -TopBar.Size.Y.Offset),
...
function redzlib:MakeTab(config)

	local Title = config.Title or "Tab"

	local TabButton = Create("TextButton", MainScroll, {
	...
	local Page = Create("Frame", Containers, {
	...
	local Content = Create("ScrollingFrame", Page, {
		Size = UDim2.new(1,0,1,0),
		BackgroundTransparency = 1,
	...
	TabButton.MouseButton1Click:Connect(function()
		for _, t in pairs(redzlib._tabs) do
			if t.Page then
	...
	function tabObj:AddSlider(opt)
		local Slider = Create("TextButton", Content, {
		...
		Slider.MouseButton1Click:Connect(function()
			if opt.Callback then
				opt.Callback(opt.Default or 100)
			end
		end)
	end

	tabObj.Page = Page
	table.insert(redzlib._tabs, tabObj)

if #redzlib._tabs == 1 then
	Page.Visible = true
end
...
local ControlSize2 = MakeDrag(Create("ImageButton", MainFrame, {
    ...
    Position = UDim2.new(0, (redzlib.Save and redzlib.Save.TabSize or 300), 1, 0),
...
local function getTabWidth()
    if MainScroll and MainScroll.Parent then
        local x = MainScroll.AbsoluteSize.X
        if x and x > 0 then return x end
    end
    return (redzlib.Save and redzlib.Save.TabSize) or 300
end

local function updateLayout()
    local tabW = getTabWidth()
    Containers.Size = UDim2.new(1, -tabW, 1, -TopBar.Size.Y.Offset)
    Containers.Position = UDim2.new(1, 0, 1, 0)
    ControlSize2.Position = UDim2.new(0, tabW, 1, 0)
end

task.defer(updateLayout)

MainScroll:GetPropertyChangedSignal("AbsoluteSize"):Connect(updateLayout)
TopBar:GetPropertyChangedSignal("AbsoluteSize"):Connect(updateLayout)
MainFrame:GetPropertyChangedSignal("AbsoluteSize"):Connect(updateLayout)
	...
	local NewTab = math.clamp(Pos2.X.Offset, 135, 250)

	ControlSize1.Position = UDim2.fromOffset(NewWidth, NewHeight)
	ControlSize2.Position = UDim2.new(0, NewTab, 1, 0)

	redzlib.Save.UISize = {NewWidth, NewHeight}
	redzlib.Save.TabSize = NewTab

	MainFrame.Size = UDim2.fromOffset(NewWidth, NewHeight)

	MainScroll.Size = UDim2.new(0, redzlib.Save.TabSize, 1, -TopBar.Size.Y.Offset)

	Containers.Size = UDim2.new(1, -redzlib.Save.TabSize, 1, -TopBar.Size.Y.Offset)

end
	
	ConnectSave(ControlSize2, function()
	redzlib.Save.TabSize = ControlSize2.Position.X.Offset
	SaveJson("redz library V5.json", redzlib.Save)
end)
...
if Layout then
	Layout:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
		MainScroll.CanvasSize = UDim2.new(0,0,0, Layout.AbsoluteContentSize.Y + 10)
	end)
end
