...
do
    ...
    local function WaitForCharacter()
        ...
        local hrp = char:FindFirstChild("HumanoidRootPart") or char:WaitForChild("HumanoidRootPart", 15)
        local energy = char:FindFirstChild("Energy") or char:WaitForChild("Energy", 15)
        return char, hrp, energy
    end

    local char, hrp, energy = WaitForCharacter()
    Root = hrp or (char and char:FindFirstChild("HumanoidRootPart"))
...
QuestNeta = function()
    ...
    do
        local ok, result = pcall(function()
            local uiModule = loadstring(game:HttpGet("https://raw.githubusercontent.com/Naknohack/RedzUi/refs/heads/main/UiRedzhubV4new.lua"))
            if not uiModule then
                error("UI module loadstring failed")
            end
            return uiModule():MakeWindow({
                Title = "Nak Hub V4",
        ...
        if ok then
            ...
            warn("[Nak Hub] Failed to create UI window:", result)
            return
        end
    end
local vu32 = Library
...
ApplyUiScale(GetSetting("UiScale_Save", "Large"))
-- Criar ScreenGui
local screenGui = Instance.new("ScreenGui")
...
FarmSettings:AddDropdown({
    ...
    Default = GetSetting("UiScale_Save", "Large"),
