...
-- =========================================================================
-- ĐOẠN CODE CUỐI HOÀN CHỈNH (DÁN TỪ DÒNG 2321 ĐẾN HẾT FILE)
-- =========================================================================

FarmSettings:AddSection({"Setting Farm"})

-- 1. Khai báo biến cốt lõi
local Players = game:GetService("Players")
local ReplicatedStorage = game:GetService("ReplicatedStorage")
local VirtualInputManager = game:GetService("VirtualInputManager")
local Player = Players.LocalPlayer
...
-- 2. Quản lý mục tiêu (Folder an toàn)
...
-- 5. Logic Fast Attack (Melee)
local AttackDelays = {
    ["Slow Attack"] = 0.25, ["Normal Attack"] = 0.15, ["Fast Attack"] = 0.05,
    ["Super Attack"] = 0.1, ["Extremely Fast Attack"] = 0.01,
    ["Bear Attack"] = 0.05, ["Super Bear Attack"] = 0.01
}

local FastAttackLogic = { Distance = 100 }

function FastAttackLogic:Attack(BasePart, OthersEnemies)
...
function FastAttackLogic:GetTargets()
    ...
    local function Scan(folder)
        ...
        for _, enemy in ipairs(folder:GetChildren()) do
            ...
            if head and IsAlive(enemy) and enemy ~= Player.Character then
                if (Player.Character.HumanoidRootPart.Position - head.Position).Magnitude < self.Distance then
    ...
    Scan(Enemies) Scan(Characters)
    return Targets, NearestPart
end

local function StopAllLoops()
    if _G.LoopFA then task.cancel(_G.LoopFA); _G.LoopFA = nil end
    if _G.LoopAC then task.cancel(_G.LoopAC); _G.LoopAC = nil end
end

local function StartFastAttack(mode)
    StopAllLoops()
    local waitTime = AttackDelays[mode] or 0.1
    _G.Settings.FastAttackMode = mode

    _G.LoopFA = task.spawn(function()
        while _G.Settings.FastAttack do
            local char = Player.Character
            if char and IsAlive(char) then
                local tool = char:FindFirstChildOfClass("Tool")
                if tool and tool.ToolTip ~= "Gun" then
                    local targets, part = FastAttackLogic:GetTargets()
                    if #targets > 0 then FastAttackLogic:Attack(part, targets) end
                end
            end
            task.wait(waitTime)
        end
...
mt.__namecall = newcclosure(function(self, ...)
    ...
    if getgenv().GunAuraSettings.GunEnabled and getgenv().GunAuraSettings.CurrentTarget then
        if method == "FireServer" and (tostring(self):find("Shoot") or tostring(self):find("Attack")) then
            if type(args[1]) == "Vector3" then args[1] = getgenv().GunAuraSettings.CurrentTarget.Position end
        end
    end
...
local function applyGunDamage()
    if not getgenv().GunAuraSettings.GunEnabled or not Player.Character then return end
    local myPos = Player.Character:GetPivot().Position
    local target, targetPart = nil, nil
    local dist = getgenv().GunAuraSettings.GunRange

    local function Check(folder)
        for _, e in ipairs(folder:GetChildren()) do
            local hrp = e:FindFirstChild("HumanoidRootPart") or e:FindFirstChild("Head")
            if hrp and IsAlive(e) and e ~= Player.Character then
                local d = (hrp.Position - myPos).Magnitude
                if d < dist then dist = d; target = e; targetPart = hrp end
            end
        end
    end
    if getgenv().GunAuraSettings.TargetType == "NPC" or getgenv().GunAuraSettings.TargetType == "All" then Check(Enemies) end
    if getgenv().GunAuraSettings.TargetType == "Player" or getgenv().GunAuraSettings.TargetType == "All" then Check(Characters) end

    getgenv().GunAuraSettings.CurrentTarget = targetPart
    if targetPart and ShootGunEvent then
        pcall(function()
            for i = 1, 3 do ShootGunEvent:FireServer(targetPart.Position, {targetPart}) end
        end)
    end
end
...
FarmSettings:AddToggle({
    ...
    Callback = function(state)
        ...
        if state then StartFastAttack(_G.Settings.FastAttackMode) else StopAllLoops() end
    end
})
...
FarmSettings:AddToggle({
    ...
    Default = false,
    Callback = function(state) _G.Settings.AutoClick = state end
})

FarmSettings:AddDropdown({
    Name = "Gun Target Type",
    Options = {"Player", "NPC", "All"},
    Default = "All",
    Callback = function(t) getgenv().GunAuraSettings.TargetType = t end
})

FarmSettings:AddToggle({
    Name = "Auto Gun Aura (Aim Bot)",
    Default = false,
    Callback = function(v) getgenv().GunAuraSettings.GunEnabled = v end
})

local targetDisplay = FarmSettings:AddParagraph({Title = "Status", Content = "Waiting..."})
...
task.spawn(function()
    while task.wait(0.5) do
        local tar = getgenv().GunAuraSettings.CurrentTarget
        if tar then
            targetDisplay:SetDesc("Aiming at: " .. tar.Parent.Name)
        else
            targetDisplay:SetDesc("Searching for targets...")
        end
    end
end)

-- THÔNG BÁO HOÀN TẤT (KHÔNG CÓ RETURN LỖI)
if NotificacaoNightMystic then
    NotificacaoNightMystic("Nak Hub", "✅ Script Fixed! Đã sửa lỗi")
end
