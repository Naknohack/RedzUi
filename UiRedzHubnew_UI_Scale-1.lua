...
function redzlib:SetScale(NewScale)
	if type(NewScale) == "string" then
		return self:SetScalePreset(NewScale)
	end
	...
	local Preset = math.clamp(tonumber(NewScale) or 450, 300, 2000)
	local ComputedScale = viewportY / Preset

	UIScale = ComputedScale
	if ScreenGui:FindFirstChild("Scale") then
		ScreenGui.Scale.Scale = ComputedScale
	end

	return ComputedScale
end

function redzlib:SetScalePreset(Name)
	local Presets = {
		["Nhỏ"] = 900,
		["Bình Thường"] = 450,
		["To"] = 350,
	...
	self.ScalePresets = Presets
	return self:SetScale(Presets[Name] or 450)
end
